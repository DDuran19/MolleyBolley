...
class LoginWindow(tk.Tk):
    ...
    def on_window_resize(self, event):
        ...
        try:
            ...
            self.customer_entry.configure(
                font=self.large_font, width=self.buttons_width
            )
            self.employees_dropdown.configure(
                font=self.large_font, width=self.buttons_width
            )
            self.assign_to_button.configure(
                font=self.large_font, width=self.buttons_width
            )
            self.mark_as_free_button.configure(
                font=self.large_font, width=self.buttons_width
            )
            self.next_in_line.configure(
            ...
            self.next_info.configure(
                font=self.large_font, bg=WHITE, width=self.buttons_width
            )

            row_height = self.large_font.metrics("linespace") + 4
            self.style.configure(
                "Custom.Treeview", rowheight=row_height, font=self.large_font
            )
            self.style.map(
                "Custom.Treeview.Heading",
                background=[("active", ROYAL_BLUE), ("hover", WHITE)],
            )
    ...
    def grid_all_buttons(self):
        self.customer_entry.grid(
            row=0, column=0, padx=0, pady=(0, 0), sticky=NORTHEASTWEST
        )
        self.next_info.grid(row=3, column=0, padx=0, pady=(0, 0), sticky=NORTHEASTWEST)
        self.next_in_line.grid(
            row=4, column=0, padx=0, pady=(10, 10), sticky=NORTHEASTWEST
        )
        self.employees_dropdown.grid(
            row=5, column=0, padx=0, pady=(10, 0), sticky=NORTHEASTWEST
        )
        self.assign_to_button.grid(
            row=6, column=0, padx=0, pady=(0, 23), sticky=NORTHEASTWEST
        )
        self.mark_as_free_button.grid(
            row=8, column=0, padx=0, pady=(0, 0), sticky=NORTHEASTWEST
        )
    ...
    def create_login_frame(self):
        ...
        self.username_label = tk.Label(
            self.frame, text="Username:", bg=ROYAL_BLUE, fg=WHITE
        )
        ...
        self.password_label = tk.Label(
            self.frame, text="Password:", bg=ROYAL_BLUE, fg=WHITE
        )
        ...
        show_password_label = tk.Label(
            self.frame, text="Show Password", bg=ROYAL_BLUE, fg=WHITE
        )
        show_password_label.grid(row=2, column=1, sticky="w", padx=(20, 0))

        self.forgot_password_label = tk.Label(
            self.frame, text="Forgot Password", fg=WHITE, bg=ROYAL_BLUE, cursor="hand2"
        )
        self.forgot_password_label.grid(row=3, columnspan=2, pady=(10, 0))
        self.forgot_password_label.bind(LEFT_CLICK, self.forgot_password)
        self.instructions_label = tk.Label(
            self.frame, text="Instructions ⓘ", bg=ROYAL_BLUE, fg=WHITE, cursor="hand2"
        )
        self.instructions_label.grid(row=4, columnspan=2, pady=(0, 10))
        self.instructions_label.bind(LEFT_CLICK, self.show_web)

        login_button = tk.Button(
            self.frame,
            text="Login",
            command=lambda: self.login(None),
            bg=ROYAL_BLUE,
            fg=WHITE,
        )
    ...
    def create_header_on_main_frame(self):
        ...
        self.image_header = self.image_header.resize(
            (resized_width, resized_height), Image.ANTIALIAS
        )
    ...
    def create_ttk_trees(self):
        ...
        self.wait_list_frame = tk.Frame(
            self.trees, borderwidth=1, relief="solid", bg=ROYAL_BLUE
        )
        self.wait_list_frame.grid(row=0, column=0, sticky=NORTHEASTWEST, pady=(0, 25))

        self.wait_list = ttk.Treeview(
            self.wait_list_frame,
            height=int(self.screen_height * 0.0095),
        ...
        self.wait_list.column("#0", width=int(self.screen_width * 0.75 / 2))
        self.wait_list.column("Service", width=int(self.screen_width * 0.75 / 2))
        ...
        self.employee_list = ttk.Treeview(
            self.wait_list_frame,
            height=int(self.screen_height * 0.005),
            style="Custom.Treeview",
        )
        ...
        self.employee_list.column(
            "Service", width=int((self.screen_width * 0.75 * 2 / 3) * 4 / 5)
        )
        self.employee_list.column(
            "Status", width=int((self.screen_width * 0.75 * 2 / 3) * 1 / 5)
        )
    ...
    def add_initial_items_on_employee_list(self):
        for employee, values in self.employees.items():
            self.employee_list.insert(
                EMPTY, tk.END, values=values, iid=employee, text=employee
            )
    ...
    def create_buttons_frame(self):
        ...
        self.customer_entry.bind(
            "<FocusIn>", lambda event: self.clear_placeholder(event=event)
        )
        self.customer_entry.bind(
            "<FocusOut>", lambda event: self.restore_placeholder(event=event)
        )
        ...
        self.employees_dropdown = ttk.Combobox(
            self.buttons_frame,
            values=self.free_employees,
            state=READONLY,
            foreground=WHITE,
            background=ROYAL_BLUE,
        )
        self.assign_to_button = tk.Button(
            self.buttons_frame,
            text="Assign-to",
            fg=WHITE,
            bg=ROYAL_BLUE,
            command=self.assign_customer_to_employee,
        )
        self.mark_as_free_button = tk.Button(
            self.buttons_frame,
            text="Mark as Free",
            fg=WHITE,
            bg=ROYAL_BLUE,
            command=self.mark_as_free,
        )
    ...
    def insert_survey_and_facebook(self):
        ...
        self.survey_label = tk.Label(
            self.socials_frame, image=self.converted_survey_logo, bg="white"
        )
        ...
        self.facebook_label = tk.Label(
            self.socials_frame, image=self.converted_facebook_logo, bg="white"
        )
        self.facebook_label.pack(side="left")

        facebook_text = "fb.com/MoleyBoley2000"
        self.facebook_text_label = tk.Label(
            self.socials_frame, text=facebook_text, bg="white", font=self.large_font
        )
    ...
    def mark_as_free(self):
        ...
        new = ServicePopup(
            self,
            False,
            self.gather_services_completed_by_employee,
            employee,
            **service_dict,
        )
    ...
    def add_customer(self, event):
        ...
        if customer_name == EMPTY:
            return
        new_popup = ServicePopup(
            self, True, self.get_service_data_to_be_updated, customer_name
        )

    def get_service_data_to_be_updated(self, query_data):
        self.customers.update()
        customer_name = (
            f"     \u2023{self.customers.get():03} {self.customer_entry.get()}"
        )
        ...
        self.wait_list.insert(
            EMPTY, END, values=(selected_service,), text=customer_name
        )
    ...
    def forgot_password(self, event):
        messagebox.showinfo(
            "Forgot Password", "Please contact the administrator for assistance."
        )

    def login(self, event):
        self.username = self.username_entry.get()
        password = self.password_entry.get()
        if self.username == EMPTY or password == EMPTY:
            return
        query = Login_query()
    ...
    * Extract All: Extracts all of the data and saves it to an Excel file.
    ...
    def create_table(self, data: pd.DataFrame):
        ...
        for index, row in data.iterrows():
            row_total = sum(row)
            self.actual_table.insert(
                "", "end", text=f"{index} ({row_total})", values=row.tolist()
            )
    ...
    def create_extract_buttons(self):
        ...
        button_style.configure(
            "Extract.TButton", foreground=WHITE, background=ROYAL_BLUE
        )
        button_style.map(
            "Extract.TButton", background=[("active", "green"), ("hover", ROYAL_BLUE)]
        )

        extract_running = ttk.Button(
            extract_frame,
            text="Extract Running Today",
            style="Extract.TButton",
            command=self.extract_running,
        )
        extract_7days = ttk.Button(
            extract_frame,
            text="Extract Last 7 Days",
            style="Extract.TButton",
            command=self.extract_last_seven_days,
        )
        extract_30days = ttk.Button(
            extract_frame,
            text="Extract Last 30 Days",
            style="Extract.TButton",
            command=self.extract_last_thirty_days,
        )
        extract_all = ttk.Button(
            extract_frame,
            text="Extract all",
            style="Extract.TButton",
    ...
    def ask_where_to_save(self, default_file_name):
        file_path = filedialog.asksaveasfilename(
            defaultextension=".xlsx", initialfile=default_file_name
        )
        return file_path
...
class ServicePopup(tk.Toplevel):
    ...
    def __init__(
        ...
        if isCustomer:
            ...
            self.title(f"Services completed - {name}")
    ...
    def setup_frame(self):
        ...
        for i, attribute in enumerate(attributes):
            checkbox = tk.Checkbutton(
                frame,
                text=attribute,
                command=lambda attr=attribute: self.update_attribute(attr),
    ...
    * A button that changes the password of the selected user.
    ...
    def create_widgets(self):
        ...
        if self.isAdmin:
            ...
            create_new_employee_label = tk.Label(
                self.adminmode, text="Create New Employee", **self.Entry_style
            )
            create_new_employee_label.grid(row=0, column=0, columnspan=2)

            employee_namelabel = tk.Label(
                self.adminmode, text="Write new employee name:", background=WHITE
            )
            ...
            username_label = tk.Label(
                self.adminmode, text="Username:", background=WHITE
            )
            ...
            password1_label = tk.Label(
                self.adminmode, text="Password:", background=WHITE
            )
            ...
            password2_label = tk.Label(
                self.adminmode, text="Confirm Password:", background=WHITE
            )
            ...
            role_checkbox = tk.Checkbutton(
                self.adminmode,
                text="Add admin privileges?",
                background=WHITE,
                variable=self.role_checkbox_var,
                command=self.set_admin_privileges,
            )
            ...
            reset_customers = tk.Button(
                self.adminmode,
                text="Reset Customer Counter",
                **self.button_style,
                command=self.reset_customers,
            )
            reset_customers.grid(row=9, column=1, sticky="w")

        if self.isAdmin == 2:
            self.current_role_checkbox_var = tk.BooleanVar()
            self.current_role_checkbox = tk.Checkbutton(
                main,
                text="Admin",
                background=WHITE,
                variable=self.current_role_checkbox_var,
                command=self.change_password,
                font=("TkDefaultFont", 15),
            )
            self.current_role_checkbox.grid(row=2, column=0, pady=0)
            self.username_delete_dropdown_var = tk.StringVar()
            self.username_delete_dropdown = ttk.Combobox(
                self.adminmode,
                state=READONLY,
                values=self.usernames,
                textvariable=self.username_delete_dropdown_var,
                font=font_style,
                width=20,
            )
            self.username_delete_dropdown.grid(row=7, column=1, pady=10)
            delete_employee = tk.Button(
                self.adminmode,
        ...
        change_password_label = tk.Label(
            main, text="Change password below", **self.Entry_style
        )
        ...
        self.confirm_password_entry.grid(
            row=4, column=0, pady=(0, 10), sticky=NORTHEASTWEST
        )

        change_password_btn = tk.Button(
            main,
            text=f'{"Change role and password" if self.isAdmin == 2 else "Change password"}',
            **self.button_style,
            font=font_style,
            command=lambda: self.change_password(
                self.password_entry.get(), self.confirm_password_entry.get()
            ),
        )
    ...
    def create_new_employee(self):
        if not self.usernameEntry.get() or not self.employee_name.get():
            ...
            messagebox.showerror(
                "Cannot be blank", "username/employee name is required!"
            )
            toggle_topmost(self)
            return
    ...
    def delete_employee(self):
        ...
        if query.delete_user(username):
            self.username_delete_dropdown["values"] = tuple(
                value
                for value in self.username_delete_dropdown["values"]
                if value != username
            )
